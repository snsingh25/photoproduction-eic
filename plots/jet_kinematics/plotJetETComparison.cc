...
// =============================================================================
// Description: Plots jet_et distributions for QQ, GG, GQ, and Combined events
...
void plotJetETComparison(bool normalize = false) {
    
    // =============================================================================
    // CONFIGURATION
    // =============================================================================
    
    const std::string inputFile = "/Users/siddharthsingh/Analysis/ph-new/subprocjets/jets-basic/dijets/eic64_dijets/dijets_eic64_R10_EtMin10.root";
    ...
    // Histogram settings
    const int nBins = 120;
    const double etMin = 0.0;  // Start from ET minimum used in analysis
    const double etMax = 100.0; // Reasonable upper limit for HERA
    ...
    for (size_t i = 0; i < categories.size(); ++i) {
        ...
        // Create histogram
        std::string histName = "h_jet_et_" + category;
        TH1F* hist = new TH1F(histName.c_str(), "", nBins, etMin, etMax);
        hist->SetLineColor(colors[i]);
        hist->SetLineWidth(2);
        hist->GetXaxis()->SetTitle("Jet E_{T} [GeV]");
        ...
        for (Long64_t j = 0; j < nEntries; ++j) {
            ...
            if (!jet_et || jet_et->empty()) continue;
            
            eventsProcessed++;
            
            // Fill histogram with all jets in the event
            for (size_t k = 0; k < jet_et->size(); ++k) {
                hist->Fill((*jet_et)[k]);
                totalJets++;
            }
        }
    ...
    for (size_t i = 0; i < categories.size(); ++i) {
        if (histograms[i]) {
            ...
            std::cout << "  Mean ET: " << histograms[i]->GetMean() << " GeV\n";
            std::cout << "  RMS: " << histograms[i]->GetRMS() << " GeV\n\n";
        }
    }
    ...
    std::string outputName = "eT_" + rootFileName + ".pdf";
    canvas->SaveAs(outputName.c_str());
    std::cout << "Canvas saved as: " << outputName << "\n";
    
    // outputName = "eT_" + rootFileName + ".png";
    // canvas->SaveAs(outputName.c_str());
    // std::cout << "Canvas saved as: " << outputName << "\n";
...
int main() {
    plotJetETComparison(false); // Default: no normalization
    return 0;
}