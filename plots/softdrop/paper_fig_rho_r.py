...
def mean_rho(psi, eta, lo, hi, min_jets=20):
    ...
    so the first bin uses just Psi(r_1)/Delta r.
    """
    m = (eta >= lo) & (eta < hi)
    if m.sum() < min_jets:
        return None
    psi_mean = psi[m].mean(axis=0)              # shape (10,)
    # Prepend Psi(0) = 0 so np.diff returns 10 values.
    psi_with_zero = np.concatenate(([0.0], psi_mean))
    return np.diff(psi_with_zero) / DELTA_R
