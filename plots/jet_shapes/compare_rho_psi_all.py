#!/usr/bin/env python3
"""Combined comparison: Psi(r) and rho(r) with all four classifications
overlaid in one panel — QQ, GG, Direct, Resolved — and all four
sqrt(s) values per classification.

For each of the 4 eta bins, produces:
    psi_all_<etabin>.pdf      (Psi(r), 16 curves: 4 classes x 4 energies)
    rho_all_<etabin>.pdf      (rho(r), same layout)

Class -> colour:
    QQ        red       (#c0392b)
    GG        blue      (#2980b9)
    Direct    green     (#27ae60)
    Resolved  purple    (#8e44ad)

sqrt(s) -> line style:  64 ":"  /  105 "--"  /  141 "-."  /  300 "-"

Inputs:
    data-jets/<sample>_antikt_dijets/   for jet observables (HERA also at
                                         anti-kT EtMin=10 so all 4 samples
                                         are compared apples-to-apples)
    data/<sample>/*.root                for event-level processCode lookup

Output: plots/jet_shapes/output/compare_rho_psi_all/
"""
...
DELTA_R = 0.1
RHO_R   = R_GRID - 0.5 * DELTA_R          # 10 centres: 0.05..0.95

# All 4 samples reconstructed with the same algorithm (anti-kT) and
# per-jet EtMin=10 so the comparison is apples-to-apples across sqrt(s).
SAMPLES = [
...
def load_sample(jet_root, ev2pc):
    """Return dict with keys 'QQ', 'GG', 'Direct', 'Resolved' →
    (psi: (N, 10), eta: (N,)) per class. Each class is a *concatenation*
    of jets across the QQ/GG/GQ trees, filtered by either:
      - tree category (QQ, GG)              for the parton-flavour classes
      - event-level processCode (Direct/Resolved) for the photon-mode classes
    """
    out = {"QQ": ([], []), "GG": ([], []),
           "Direct": ([], []), "Resolved": ([], [])}
    ...
    with uproot.open(jet_root) as f:
        for cat in ("QQ_Events", "GG_Events", "GQ_Events"):
            ...
            # Photon-mode class (Direct/Resolved) — needs processCode per jet
            pc_per_event = np.array([ev2pc.get(int(e), -1) for e in ev_ids],
                                    dtype=np.int32)
            pc_per_jet = np.repeat(pc_per_event, n_jets)
            direct_mask   = np.isin(pc_per_jet, list(DIRECT_CODES))
            resolved_mask = np.isin(pc_per_jet, list(RESOLVED_CODES))
            if direct_mask.any():
                out["Direct"][0].append(psi[direct_mask])
                out["Direct"][1].append(eta[direct_mask])
            if resolved_mask.any():
                out["Resolved"][0].append(psi[resolved_mask])
                out["Resolved"][1].append(eta[resolved_mask])
...
def plot_panel(curves_by_class_sqrts, eta_label, out_pdf, *,
               kind="psi"):
    """curves_by_class_sqrts: {class_name: {sqrts: numpy(10,)}}"""
    fig, ax = plt.subplots(figsize=(9, 8))
    r_smooth = np.linspace(RHO_R.min(), RHO_R.max(), 300) if kind == "rho" \
               else np.linspace(R_GRID.min(), R_GRID.max(), 300)

    for cls in ("QQ", "GG", "Direct", "Resolved"):
    ...
    if kind == "psi":
        ...
        ax.set_xlim(0.0, 1.0)            # visual gap from 0 to 0.05
    ...
    # Eta label goes to the right side where the curves have decayed —
    # the upper-left is occupied by the QQ peak for rho(r) plots.
...
def main():
    setup_style()
    out_dir = REPO / "plots" / "jet_shapes" / "output" / "compare_rho_psi_all"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("Loading samples (jet ROOT + processCode lookup):")
    sample_data = {}  # sqrts -> {class: (psi, eta)}
    ...
    for lo, hi, label, slug in ETA_BINS:
        # collect curves keyed (class -> sqrts -> psi)
        curves = {cls: {} for cls in ("QQ", "GG", "Direct", "Resolved")}
        ...
        plot_panel(curves, label, str(out_dir / f"psi_all_{slug}.pdf"),
                   kind="psi")
        plot_panel(curves, label, str(out_dir / f"rho_all_{slug}.pdf"),
                   kind="rho")
